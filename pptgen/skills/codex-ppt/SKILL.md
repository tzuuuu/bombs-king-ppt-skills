---
name: codex-ppt
description: Generate visually unified full-slide images and reproducible Project Workspaces from articles, reports, papers, notes, or outlines.
metadata:
  openclaw:
    requires:
      bins:
...
This skill creates complete 16:9 slide images from source material and delivers them as a validated Project Workspace for downstream editable-PowerPoint reconstruction. It does not assemble an intermediate image-based `.pptx`.
...
- Respect approval gates. Do not create final `deck_spec.json`, `speech.md`, prompt jobs, slide images, or handoff state before the approvals in `docs/workflow-gates-and-progress.md`.
...
- The main agent owns orchestration, prompt jobs, state recording, QA, speaker notes, and Project Workspace handoff. Do not silently replace available slide subagents with sequential production.
...
- Every Data Chart must be generated during the task from structured data with Python and delivered as a Chart Source Package containing the generator, exact CSV/JSON snapshot, and transparent PNG render.
- Use only the managed Matplotlib, pandas, NumPy, and Seaborn chart toolchain unless the user explicitly approves another dependency. Chart generators must not install packages or fetch live data when reproducing the delivered render.
- Chart renders contain only the chart marks and necessary axes, scales, legends, units, and labels. Do not add slide titles, narrative copy, page numbers, cards, borders, shadows, or decorative containers.
- The image backend generates the complete slide using each chart render as required visual context. Do not reserve placeholders or locally composite chart renders into final slide images.
- The Slide Image Set is authoritative for layout and style; Chart Source Packages are authoritative for Data Chart values.
...
6. Create the project directory.
   - Before initializing folders or validating handoff files, read `docs/project-handoff-and-reporting.md`.
...
8. Generate Data Charts when required.
   - For every Data Chart in the approved outline, create its Chart Source Package and deck-level `chart_manifest.json` before preparing slide jobs.
   - Generate the transparent chart render from the delivered local snapshot with the managed Python chart toolchain. Treat the package as required downstream input.
...
11. Quality check and repair.
    - Before QA or handoff, read `docs/project-handoff-and-reporting.md`.
    - Inspect every slide before handoff: text, outline match, truncation, style, unwanted page numbers, overlaps, and required assets.
...
12. Write speaker notes and validate the Project Workspace.
    - Before writing `speech.md` or validating handoff, read `docs/project-handoff-and-reporting.md`.
    - Make sure `outline.md` reflects the final confirmed deck outline. Use `speech.md` headings that map to `Slide N`.
    - Run `scripts/project_handoff.py validate {base_dir}/{deck_name}` only after `slide_jobs.json` shows generated slides as `recorded` and approved samples as `accepted`. If any slide is `pending`, `dispatched`, or `blocked`, stop.

13. Report the result.
    - Use the final report checklist in `docs/project-handoff-and-reporting.md`.
...
14. Save reusable styles.
    ...
    - If the final deck used a custom or adapted style, proactively offer to save it in the final report, per `docs/project-handoff-and-reporting.md`. User custom styles are stored in `${CODEX_PPT_HOME:-~/.codex-ppt-skill}/references/`, outside the skill install.
...
Subagents must not edit `outline.md`, `deck_spec.json`, `chart_manifest.json`, Chart Source Packages, other slide jobs, `origin_image/`, or `speech.md`. The parent records outputs and validates the final handoff.

## Acceptance Criteria

- Output is a validated Project Workspace with no intermediate `.pptx`.
...
- `speech.md` exists when speaker notes are expected and remains available for downstream reconstruction.
- `slide_jobs.json` and `slide_run_state.json` reflect the final state.
- Every planned Data Chart has exactly one valid Chart Source Package and `chart_manifest.json` entry.
- Every chart generator, data snapshot, and transparent chart render is present; chart manifest paths are workspace-relative and contain no placement coordinates.
...
- `docs/project-handoff-and-reporting.md`: project directory, notes, handoff validation, final report, prompting principles.
