...
5. **Sample gate — approve one representative page.**
   Follow the sample branch in `docs/outline-style-and-sample.md`. Read `docs/project-handoff-and-reporting.md`, initialize the Project Workspace with `scripts/project_handoff.py init`, and select the representative page. If that page plans any Data Chart, read `docs/data-charts.md` and finish every Chart Source Package and its `chart_manifest.json` entry before generating the sample. Then generate exactly one representative `origin_image/slide_XX.png` without a visible page number. When a master template exists, include its selected `template/template-<N>.png` as a strict input. Show it, iterate on that same page, and record its approved `sample_generation_method` in `deck_spec.json`. Default to the current or source directory when no destination is given.
   **Complete when:** every Data Chart used by the sample already reproduces from its local snapshot and has a valid manifest entry, the user approves the page, and the recorded method identifies the actual backend, tool, mode, prompt source, approved image, and available generation settings.

6. **Package gate — build reproducible inputs.**
   Prepare strict user assets per `docs/user-supplied-assets.md`. When the approved outline plans any Data Chart, read `docs/data-charts.md` and finish every remaining Chart Source Package plus the deck-level `chart_manifest.json` before prompt preparation; preserve and validate any sample-chart entries completed at Gate 5. When page-scoped package, asset, or context tasks are independent, use subagents to parallelize all of them by page and fill every available slot; keep shared manifest assembly and validation in the parent.
