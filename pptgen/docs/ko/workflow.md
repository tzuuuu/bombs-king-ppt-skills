...
agent는 먼저 소스 브리프와 자산 목록을 보여 줍니다. 각 자료에는 출처 또는 경로, 예정 용도, 현재 사용 가능 상태를 기록합니다. 핵심 브리프 필드에는 명시적 값 또는 사용자가 승인한 가정이 필요하며, 미해결 질문이 있으면 이 단계는 완료되지 않습니다.
...
샘플 슬라이드 확인 후 agent는 생성 대기 중인 각 페이지에 하위 agent 하나씩을 배정하고 사용 가능한 worker 슬롯을 모두 채웁니다. 이 skill의 페이지 worker 기본 용량은 30입니다. worker가 반환되면 agent는 결과를 즉시 기록하고 다음 대기 페이지를 비워진 슬롯에 바로 배정하여 사용 가능한 용량이나 페이지 작업이 놀지 않게 합니다. 모든 페이지는 승인된 샘플 슬라이드와 동일한 스타일 및 이미지 생성 백엔드를 사용합니다.
