...
```text
pptgen skill을 사용해서 이 자료를 10페이지 PPT로 만들고, 내장된 「손그림 기술 설명 스타일」을 사용해 주세요.
```
...
```text
pptgen skill을 사용해 PPT를 생성해 주세요. 시각 스타일은 제가 업로드한 PDF를 참고하세요. 제공한 자료의 각 페이지 이미지를 자세히 읽고 스타일을 충분히 이해한 뒤, 그 스타일을 재현해 생성해 주세요.
```
