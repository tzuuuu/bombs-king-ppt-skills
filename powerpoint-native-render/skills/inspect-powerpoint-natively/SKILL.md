...
A healthy diagnostic proves only that the local runtime prerequisites are present. It is not evidence that a presentation was rendered or inspected. If the installed CLI does not expose the later render-and-verify commands required to seal that evidence, report the native render gate as unavailable.
